    ...
    return (
      <div className="nexo-page">
        ...
        <div className="nexo-card" style={{ padding: 20 }}>
          <p>登录后查看订单</p>
          <Link to="/me" className="nexo-btn-primary" style={{ marginTop: 12, textDecoration: 'none' }}>
            去登录
          </Link>
        </div>
      </div>
    ...
    <div className="nexo-page">
      ...
      <div style={{ display: 'flex', gap: 6, overflowX: 'auto', marginBottom: 14, paddingBottom: 4 }}>
        {tabs.map((t) => (
          <button
            ...
            className={tab === t.key ? 'nexo-chip' : 'nexo-chip-blue'}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {list.map((o) => {
          const f = o.flights[0];
          return (
            <div key={o.id} className="nexo-card" style={{ padding: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                <span style={{ fontWeight: 700 }}>
                  {f ? `${f.dep_city} → ${f.arr_city}` : '订单'}
                </span>
                <span className="nexo-chip-blue">{o.status}</span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                {o.status === 'pending' && (
                  <Link to={`/orders/${o.id}/pay`} className="nexo-btn-primary" style={{ flex: 1, textDecoration: 'none' }}>
                    去支付
                  </Link>
                )}
                <Link to={`/orders/${o.id}`} className="nexo-btn-secondary" style={{ flex: 1, textDecoration: 'none' }}>
                  详情
                </Link>
              </div>
            </div>
      ...
      {list.length === 0 && !err && <div className="nexo-loading">暂无订单</div>}
    </div>
  );
}
